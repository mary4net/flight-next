...
async function getBookedInfo(request) {
    ...
    if (isNaN(parseInt(id))) {
      return NextResponse.json({"error": 'Invalid authorId parameter. It must be a number.' }, { status: 400 });
    }
...
async function cancelBooking(request) {
    ...
    if (booking.status !== "CONFIRMED") {
        return NextResponse.json({ "error": "Booking is not confirmed or cancelled already."}, { status: 400 });
    }

    if (!cancelHotel && !cancelFlight) {
        return NextResponse.json({ "error": "At least one of cancelHotel or cancelFlight must be provided."}, { status: 400 });
    }
