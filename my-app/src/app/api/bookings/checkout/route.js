...
async function checkout(request) {
    let { cardNumber, expiryMonth, expiryYear, cvv, booking, passportNumber} = await request.json();
    ...
    if (isNaN(parseInt(cardNumber)) || isNaN(parseInt(expiryMonth)) || isNaN(parseInt(expiryYear)) || isNaN(parseInt(cvv))) {
        return NextResponse.json({ error: "Invalid card format." }, { status: 400 });
    }
    ...
    if (parseInt(expiryMonth) < 1 || parseInt(expiryMonth) > 12) {
        return NextResponse.json({ error: "Invalid expiry month. It must be between 01 and 12." }, { status: 400 });
    }

    const currentYear = new Date().getFullYear() % 100; // Get last two digits of the current year
    if (parseInt(expiryYear) < currentYear) {
        return NextResponse.json({ error: "Card expiry year cannot be in the past." }, { status: 400 });
    } else if (parseInt(expiryYear) === currentYear && parseInt(expiryMonth) < new Date().getMonth() + 1) {
        return NextResponse.json({ error: "Card expiry month cannot be in the past." }, { status: 400 });
    }
    ...
    try {
        ...
        if (booking.itinerary.includes("HOTEL")) {
            if (await checkRoomBooked(booking.room, booking.checkIn, booking.checkOut)){
                return NextResponse.json({ error: "Room is already booked on selected time." }, { status: 400 });
            }
        ...
        if (booking.itinerary.includes("FLIGHT") || booking.itinerary.includes("AND")) {
            ...
            let response = await fetch('https://advanced-flights-system.replit.app/api/bookings', {
                method: 'POST',
                headers: {
            ...
            if (response.ok) {
                ...
                return NextResponse.json({ error: response?.error || "Error booking flight." }, { status: 400 });
            }
            ...
            flightcost += response.flights.reduce((sum, flight) => sum + flight.price, 0);;
        ...
        if (booking.itinerary.includes("FLIGHT")){
            ...
            const noti2 = await prisma.Notification.create({
                data:{
