...
async function createBooking(request) {
    ...
    if (
        (flights.length === 0 && (itinerary.includes("FLIGHT") || itinerary.includes("AND"))) ||
        (hotelLength === 0 && itinerary.includes("HOTEL"))
    ) {
        return NextResponse.json({ error: "Required data missing" }, { status: 400 });
    }

    if (hotelLength > 0 && (!hotelRoom.id || !hotelRoom.checkIn || !hotelRoom.checkOut || !hotelRoom.price)) {
        return NextResponse.json({ error: "Missing hotel room data" }, { status: 400 });
    }    

    hotelRoom.checkIn = new Date(hotelRoom.checkIn);
    hotelRoom.checkOut = new Date(hotelRoom.checkOut);

    if (hotelLength > 0 && (isNaN(hotelRoom.price) || isNaN(hotelRoom.checkIn.getTime()) || isNaN(hotelRoom.checkOut.getTime()) || isNaN(hotelRoom.id))) {
        return NextResponse.json({ error: "Invalid hotel room data type." }, { status: 400 });
    }

    if (hotelLength > 0 && (hotelRoom.price < 0 || hotelRoom.checkIn < new Date() || hotelRoom.checkOut < hotelRoom.checkIn)) {
        return NextResponse.json({ error: "Invalid hotel room data" }, { status: 400 });
    }

    // Check if hotel room is already booked
    if (hotelLength > 0 && (await checkRoomBooked(hotelRoom, hotelRoom.checkIn = new Date(hotelRoom.checkIn), hotelRoom.checkOut = new Date(hotelRoom.checkOut)))) {
        return NextResponse.json({ error: "Room is not available on selected time." }, { status: 400 });
    }
    ...
    for (const flight of flights) {
        ...
        if (!flight.departureTime || isNaN(new Date(flight.departureTime).getTime())) {
        ...
        if (!flight.price || typeof flight.price !== 'number' || flight.price < 0) {
            return NextResponse.json({ error: "Invalid flight price" }, { status: 400 });
        }

        if (!flight.origin || !flight.origin.code || !flight.origin.name || !flight.origin.city || !flight.origin.country) {
            return NextResponse.json({ error: "Invalid flight origin information" }, { status: 400 });
        }

        if (!flight.destination || !flight.destination.code || !flight.destination.name || !flight.destination.city || !flight.destination.country) {
            return NextResponse.json({ error: "Invalid flight destination information" }, { status: 400 });
        }

        if (!flight.airline || !flight.airline.code || !flight.airline.name) {
            return NextResponse.json({ error: "Invalid airline information" }, { status: 400 });
        }
    }
    ...
    try {
        let newBooking = await prisma.booking.create({
            data: {
                userId: user.id,
                itinerary: itinerary,
                hotelCost: hotelLength > 3 ? hotelRoom.price * ((new Date(hotelRoom.checkOut) - new Date(hotelRoom.checkIn)) / (1000 * 60 * 60 * 24)) : null,
...
async function updateBooking(request) {
    ...
    */
    ...
    if (booking.status === "PENDING") {
        ...
        if (addFlight.length > 0) {
            ...
            if (booking.itinerary.includes("HOTEL")) {
                if (addFlight.length === 2 && addFlight[0].origin.city === addFlight[1].destination.city) {
            ...
            for (const flight of addFlight) {
                ...
                let flightRecord = existingFlight || await prisma.flight.create({
                        data: {
                            flightId: flight.id,
                            flightNum: flight.flightNumber,
                            departureTime: new Date(flight.departureTime),
        ...
        if (addHotel && Object.keys(addHotel).length > 0) {
            ...
            if (!addHotel.id || !addHotel.checkIn || !addHotel.checkOut || !addHotel.price) {
                return NextResponse.json({ error: "Missing or invalid hotel data" }, { status: 400 });
            }
            ...
            if (addHotel.checkIn >= addHotel.checkOut || addHotel.checkIn < new Date()) {
                return NextResponse.json({ error: 'Invalid hotel check-in and check-out dates.' }, { status: 400 });
            }

            const isRoomBooked = await checkRoomBooked(addHotel, addHotel.checkIn, addHotel.checkOut);
            if (isRoomBooked) {
                return NextResponse.json({ error: 'Room is already booked.' }, { status: 400 });
            }
    
            // Prepare updated hotel information
            const nights = (addHotel.checkOut - addHotel.checkIn) / (1000 * 60 * 60 * 24);
            const hotelCost = (addHotel.price * nights) > 0 ? (addHotel.price * nights) : null;

            if (!hotelCost) {
                return NextResponse.json({ error: 'Invalid hotel cost.' }, { status: 400 });
            }
            ...
            if (!updatedBookingData.hotelCost || !updatedBookingData.checkIn || !updatedBookingData.checkOut) {
                return NextResponse.json({ error: 'Invalid hotel data.' }, { status: 400 });
            }
    ...
    return NextResponse.json({ error: "You're trying to update a non-pending booking." }, { status: 400 });
}
