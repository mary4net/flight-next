...
async function getSuggestions(request) {
    ...
    switch (itinerary) {
        ...
        case "FLIGHT_ROUNDTRIP":
            if (!flightDestination) return NextResponse.json({ error: "Flight not found"}, { status: 404 });
        ...
        case "HOTEL_RESERVATION":
            ...
            try {
                ...
                return NextResponse.json({ error: "Failed to fetch flight suggestions" }, { status: 500 });
            }
        case "ONEWAY_AND_HOTEL":
        case "ROUNDTRIP_AND_HOTEL":
            return NextResponse.json({ message: "No suggestions available for this itinerary type" }, { status: 200 });
        default:
