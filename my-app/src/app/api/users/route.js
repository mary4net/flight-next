  ...
  try {
    ...
    if (profilePicture) {
      try {
        ...
        return NextResponse.json({ error: "Profile picture must be a valid JSON" }, { status: 400 });
      }
    }
    ...
    const response = NextResponse.json({ message: "User created successfully", user: account });
...
async function PUT_edit(request) {
  try {
    ...
    if (!firstName && !lastName && !email && !phoneNumber && !profilePicture) {
      return NextResponse.json({ error: 'No profile data to update' }, { status: 400 });
    }
...
async function GET_acc(request) {
  try {
    ...
    return NextResponse.json({ message: 'Error getting profile' }, { status: 500 });
  }
}
