...
import { setCookie, comparePassword, generateToken, withAuth } from "@/utils/auth";
...
export async function POST(request) {
  ...
  try{
    ...
    if (user == null) {
      return NextResponse.json({error: "email needs to be registered"}, {status: 401});
    }
    ...
    const response = NextResponse.json({ message: "User login successfully", user: user});
