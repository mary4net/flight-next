...
export async function GET(request) {
  try {
    ...
    if (typeof(desti)!=="string"){
      return NextResponse.json({ error: 'type of destination must be string' }, { status: 400 });
    }
    ...
    const matchingCities = city.filter(c => c.city.startsWith(desti));
    const matchingAirports = airport.filter(a => a.name.startsWith(desti) || a.city.startsWith(desti));

    return NextResponse.json({matchingCities, matchingAirports});

  } catch (error) {
    console.log('fetch destination city/airport error', error);
    return NextResponse.json({ message: 'fetch destination city/airport error' }, { status: 500 });
  }
}
