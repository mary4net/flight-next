...
export async function GET(request) {
  try {
    ...
    if (!origin || !destination || !date) {
      return NextResponse.json({ error: 'Missing parameter to search' }, { status: 400 });
    }

    if (typeof(origin) !== "string" || typeof(destination) !== "string"){
      return NextResponse.json({ error: 'Type of origin and destination must be string' }, { status: 400 });
    }
    ...
    if (!origin_in_city && !origin_in_airport){
      return NextResponse.json({ error: 'Can\'t find origin in database' }, { status: 403 });
    }
    if (!destination_in_city && !destination_in_airport){
      return NextResponse.json({ error: 'Can\'t find destination in database' }, { status: 404 });
    }
    ...
    if (round && JSON.parse(round.toLowerCase())) {
      ...
      if (dates.length == 1) {
        return NextResponse.json(
          { error: 'Only 1 date recieved, expecting 2 for round trip' }, { status: 401 }
        );
      } else if (dates.length > 2) {
        return NextResponse.json(
          { error: 'Too many dates, expecting 2 for round trip' }, { status: 402 }
        );
      }
      ...
      const queryParams2 = new URLSearchParams(
        { origin:destination, destination:origin, date:dates[1] }
      );
    ...
    return NextResponse.json({ message: 'fetch flight error' }, { status: 500 });
  }
}
